...
class SecuritiesCollateralLoanVisualizer:
    ...
    def generate_all_charts(
        ...
        *,
        ...
        if isinstance(asset_breakdown, pd.DataFrame) and not asset_breakdown.empty:
            try:
                chart_paths["allocation"] = self._plot_collateral_allocation(asset_breakdown)
            except Exception:
                pass
        ...
        if scenario_df is not None and not scenario_df.empty:
            try:
                chart_paths["scenarios"] = self._plot_stress_scenarios(
                    scenario_df, warning_ratio=warning_ratio, liquidation_ratio=liquidation_ratio
                )
            except Exception:
                pass
        ...
        if portfolio_series is not None and not portfolio_series.empty:
            try:
                chart_paths["portfolio_value"] = self._plot_portfolio_value(portfolio_series)
            except Exception:
                pass
    ...
    def _plot_collateral_allocation(self, asset_breakdown: pd.DataFrame) -> Path:
        ...
        for idx, (ticker, weight) in enumerate(zip(df["ticker"], df["weight"])):
            ax.text(weight * 100 + 0.5, idx, f"{weight * 100:.1f}%", va="center", fontsize=9)
    ...
    def _plot_loan_ratio(
        ...
        *,
        ...
        ax.plot(data.index, data.values, color="#005f99", linewidth=1.6, label="Loan Ratio")
    ...
    def _plot_stress_scenarios(
        ...
        *,
        ...
        ax.axhline(warning_ratio * 100, color="#ff7f0e", linestyle="--", linewidth=1.1, label="Margin Call")
        ax.axhline(liquidation_ratio * 100, color="#d62728", linestyle="--", linewidth=1.1, label="Liquidation")
        ax.set_ylabel("Loan Ratio (%)")
        ax.set_title("Stress Scenario Loan Ratios")
        ax.legend(frameon=False)

        for idx, value in enumerate(df["loan_ratio"]):
            ax.text(idx, value * 100 + 1.5, f"{value * 100:.1f}%", ha="center", fontsize=9)
    ...
    @staticmethod
    def _coerce_frame(data: Iterable[dict] | pd.DataFrame | None) -> pd.DataFrame | None:
        if data is None:
            return None

...
from ai_trading_crew.use_cases.index_7_portfolio.analysis import Index7PortfolioAnalyzer
from ai_trading_crew.use_cases.index_7_portfolio.config import Index7PortfolioConfig
from ai_trading_crew.use_cases.index_7_portfolio.data_pipeline import Index7PortfolioDataPipeline
...
class Index7PortfolioValidator:
    ...
    def walk_forward_test(
        ...
        if total_years < train_years + test_years:
            raise ValueError(f"データ期間不足: {total_years:.1f}年 < {train_years + test_years}年")
        ...
        while current_train_end + pd.DateOffset(years=test_years) <= end_date:
            ...
            portfolio_weights = optimization_result["portfolio"].set_index("ticker")["weight"].to_dict()
    ...
    def _simulate_period(
        ...
        weights = {ticker: initial_weights.get(ticker, 0.0) for ticker in prices.columns}
    ...
    def benchmark_comparison(self) -> Dict:
        ...
        optimized_weights = optimized_result["portfolio"].set_index("ticker")["weight"].to_dict()

        equal_weights = {ticker: 1.0 / len(prices.columns) for ticker in prices.columns}

        equity_tickers = [
            ticker for ticker in prices.columns
            if index_master[index_master["ticker"] == ticker]["category"].values[0] == "equity"
        ]
        bond_tickers = [
            ticker for ticker in prices.columns
            if index_master[index_master["ticker"] == ticker]["category"].values[0] == "bonds"
        ]
    ...
    def stress_test(self) -> Dict:
        ...
        for period_name, start, end in stress_periods:
            try:
                ...
                perf = self._simulate_period(period_prices, optimized_weights, rebalance_freq="M")
    ...
    def sensitivity_analysis(self) -> Dict:
        ...
        configs = {
            ...
            "max_volatility_20pct": self._modify_config(base_config, "max_volatility", 0.20),
            "sharpe_weight_80pct": self._modify_config(base_config, "sharpe_weight", 0.80),
        }

        results = {}
        for config_name, config in configs.items():
            analyzer = Index7PortfolioAnalyzer(config)
            analysis_result = analyzer.evaluate({"prices": prices, "index_master": index_master})
            weights = analysis_result["portfolio"].set_index("ticker")["weight"].to_dict()
    ...
    def _modify_config(self, config: Index7PortfolioConfig, param: str, value: float) -> Index7PortfolioConfig:
        import copy
    ...
    def generate_validation_report(self, output_dir: Path) -> Path:
        report_lines = ["# Index 7-Portfolio Validation Report\n"]
        report_lines.append(f"**Generated:** {pd.Timestamp.now()}\n\n")

        report_lines.append("## 1. Walk-Forward Test\n")
        wf_results = self.walk_forward_test(train_years=5, test_years=2, rebalance_freq="Q")
        summary = wf_results["summary"]
        report_lines.append(f"- **Periods tested:** {summary['num_periods']}\n")
        report_lines.append(f"- **Avg Out-of-Sample Sharpe:** {summary['avg_sharpe']:.3f} ± {summary['std_sharpe']:.3f}\n")
        report_lines.append(f"- **Avg Annual Return:** {summary['avg_annual_return']*100:.2f}%\n")
        report_lines.append(f"- **Avg Max Drawdown:** {summary['avg_max_drawdown']*100:.2f}%\n")
        report_lines.append(f"- **Stability Score:** {summary['stability_score']:.3f}\n\n")

        report_lines.append("## 2. Benchmark Comparison\n")
        ...
        report_lines.append("## 3. Stress Test\n")
        ...
        for period_name, perf in stress_results.items():
            ...
            if "error" in perf:
                ...
                report_lines.append(f"- **Total Return:** {perf['total_return']*100:.2f}%\n")
                report_lines.append(f"- **Max Drawdown:** {perf['max_drawdown']*100:.2f}%\n")
                report_lines.append(f"- **LTV Warning Breaches:** {perf['ltv_breaches']['warning']}\n")
                report_lines.append(f"- **LTV Liquidation Breaches:** {perf['ltv_breaches']['liquidation']}\n\n")

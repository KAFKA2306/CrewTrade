...
def build_insight_markdown(analysis_payload: Dict[str, pd.DataFrame]) -> str:
    ...
    lines.append(_format_table(signal_counts, ["ticker", "signals"], ["Ticker", "Signals"]))
    lines.append("")
    lines.append("## Gap Magnitude")
    abs_stats = edges.groupby("ticker")["abs_gap_pct"].agg(["count", "mean", "median", "max"]).reset_index()
    abs_stats = abs_stats.rename(columns={"count": "signals", "mean": "mean_abs_gap_pct", "median": "median_abs_gap_pct", "max": "max_abs_gap_pct"})
    ...
    lines.append(_format_table(abs_stats, ["ticker", "signals", "mean_abs_gap_pct", "median_abs_gap_pct", "max_abs_gap_pct"], ["Ticker", "Signals", "Mean %", "Median %", "Max %"]))
    lines.append("")
    ...
    if not recent.empty:
        ...
        recent_mean = recent.groupby("ticker")["abs_gap_pct"].mean().reset_index(name="mean_abs_gap_pct")
        recent_mean["mean_abs_gap_pct"] = (recent_mean["mean_abs_gap_pct"] * 100).round(2)
        recent_stats = pd.merge(recent_counts, recent_mean, on="ticker", how="outer").fillna(0)
        lines.append(_format_table(recent_stats, ["ticker", "signals", "mean_abs_gap_pct"], ["Ticker", "Signals", "Mean %"]))
    else:
    ...
    lines.append(_format_table(streak_summary, ["ticker", "max_length", "streaks_ge_2", "streaks_ge_3"], ["Ticker", "Max Days", "Streaks ≥2", "Streaks ≥3"]))
    lines.append("")
    ...
    for threshold, table in threshold_tables.items():
        ...
        if table.empty:
            ...
            lines.append(_format_table(table, ["ticker", "count", "mean", "median", "max"], ["Ticker", "Count", "Mean Days", "Median Days", "Max Days"]))
        lines.append("")
...
def _compute_streak_summary(edges: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker, group in edges.sort_values("date").groupby("ticker"):
        ...
        for _, row in group.iterrows():
            ...
            if prev_date is None or current_date != prev_date + pd.Timedelta(days=1) or current_direction != prev_direction:
                if streak_length > 0:
        ...
        if not streak_lengths:
            summary_records.append({"ticker": ticker, "max_length": 1, "streaks_ge_2": 0, "streaks_ge_3": 0})
        else:
            ...
            summary_records.append({"ticker": ticker, "max_length": max_length, "streaks_ge_2": streaks_ge_2, "streaks_ge_3": streaks_ge_3})
...
def _compute_reversion_tables(metrics: pd.DataFrame) -> Dict[float, pd.DataFrame]:
    ...
    for ticker in metrics.columns.levels[0]:
        ...
        for idx, start_row in flagged.iterrows():
            ...
            for threshold in thresholds:
                ...
                if duration >= 0:
                    records.append({"ticker": ticker, "threshold": threshold, "duration": duration})
    ...
    for threshold in thresholds:
        ...
        if subset.empty:
            ...
            stats = subset.groupby("ticker")["duration"].agg(["count", "mean", "median", "max"]).reset_index()

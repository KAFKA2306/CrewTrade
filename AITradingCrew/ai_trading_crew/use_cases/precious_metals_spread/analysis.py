...
import pandas as pd
from ai_trading_crew.use_cases.precious_metals_spread.config import PreciousMetalsSpreadConfig, ETFInstrument
...
class PreciousMetalsSpreadAnalyzer:
    ...
    def evaluate(self, data_payload: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        theoretical_prices = self._compute_theoretical_prices(aligned_frames, self.config.etf_instruments.values())
    ...
    def _align_frames(self, data_payload: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    ...
    def _compute_theoretical_prices(self, aligned_frames: Dict[str, pd.DataFrame], instruments: List[ETFInstrument]) -> pd.DataFrame:
    ...
    def _compute_metrics(self, etf_frame: pd.DataFrame, theoretical_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker in theoretical_frame.columns:
            ...
            rolling_mean = gap.rolling(window=self.config.rolling_window, min_periods=1).mean()
            rolling_std = gap.rolling(window=self.config.rolling_window, min_periods=1).std()
    ...
    def _detect_edges(self, metrics_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(records) == 0:
            return pd.DataFrame(columns=["date", "ticker", "gap", "gap_pct", "z_score", "direction"])

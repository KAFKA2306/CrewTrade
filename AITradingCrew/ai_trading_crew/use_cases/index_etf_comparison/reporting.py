...
class IndexETFComparisonReporter:
    def __init__(self, config: IndexETFComparisonConfig, processed_dir: Path, report_dir: Path) -> None:
        self.config = config
    ...
    def _build_report(self, index_name: str, metrics: pd.DataFrame) -> str:
        ...
        lines.append(f"- データポイント範囲: {metrics['data_points'].min():.0f} - {metrics['data_points'].max():.0f}日")
        lines.append("")

        lines.append("## ETF比較表")
        lines.append("| ランク | ティッカー | 名称 | 運用会社 | 経費率 | 年率リターン | 年率ボラティリティ | シャープ比 | 平均出来高 |")
        lines.append("| --- | --- | --- | --- | --- | --- | --- | --- | --- |")

        for rank, (_, row) in enumerate(metrics.iterrows(), 1):
            expense = row.get("expense_ratio")
            expense_display = f"{expense * 100:.2f}%" if expense is not None and pd.notna(expense) else "N/A"
        ...
        if not metrics.empty and "expense_ratio" in metrics.columns:
            lines.append("## 経費率ランキング")
            expense_sorted = metrics.dropna(subset=["expense_ratio"]).sort_values("expense_ratio")
        ...
        lines.append("## サマリー統計")
        lines.append(f"- 平均年率リターン: {metrics['annual_return'].mean() * 100:.2f}%")
        lines.append(f"- 平均年率ボラティリティ: {metrics['annual_volatility'].mean() * 100:.2f}%")
    ...
    def _build_summary(self, index_results: Dict[str, Dict[str, Any]]) -> str:
        lines = []
        lines.append("# Index ETF Comparison - 全体サマリー")
        lines.append("")
        lines.append("## 指数別ETF数")

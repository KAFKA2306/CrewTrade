...
class JPXETFExpenseRatioClient:
    ...
    def _download_and_parse(self) -> pd.DataFrame:
        ...
        combined["expense_ratio"] = combined[self.RAW_COLUMN_EXPENSE].map(self._parse_expense_ratio)

        result = combined[["ticker", "expense_ratio"]].dropna(subset=["ticker"]).drop_duplicates("ticker", keep="first")

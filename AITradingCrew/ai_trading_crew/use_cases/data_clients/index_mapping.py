...
class IndexETFMappingClient:
    def __init__(self, raw_data_dir: Path, index_keywords: Dict[str, List[str]]) -> None:
    ...
    def get_mapping(self, force_refresh: bool = False) -> pd.DataFrame:
        ...
        for index_name, keywords in self.index_keywords.items():
            matched_etfs = self._match_etfs(etf_master, keywords)
    ...
    def _match_etfs(self, etf_master: pd.DataFrame, keywords: List[str]) -> List[str]:
        matched = set()

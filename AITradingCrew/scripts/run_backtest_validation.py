...
for year in range(start_year, end_year + 1):
    ...
    processed_dir = Path(f'resources/data/use_cases/securities_collateral_loan/processed/{date_str}')
    ...
    try:
        ...
        if 'category' in asset_breakdown.columns:
            cat_weights = asset_breakdown.groupby('category')['market_value'].sum() / asset_breakdown['market_value'].sum()
...
results_path = Path('output/backtests/securities_collateral_loan/validation_results.csv')

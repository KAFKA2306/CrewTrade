...
class TwelveTI:
    ...
    def _get_data(self, period="4mo"):
        self._data = twelve_data_manager.get_time_series_data(
            self.symbol, self.interval, period
        )
        column_mapping = {
            "open": "Open",
            "high": "High",
    ...
    def _get_column_data(self, data, column_name):
        ...
        if column_name not in data.columns:
            ...
            raise KeyError(
                f"Column '{column_name}' not found in data. Available columns: {available_columns}. "
    ...
    def fetch_macd(self, macd_fast_period: int = 12, macd_slow_period: int = 26):
        ...
        macd, macdsignal, macdhist = talib.MACD(
            close, fastperiod=fp, slowperiod=sp, signalperiod=sig
        )
        v, s, h = macd[-1], macdsignal[-1], macdhist[-1]
        last_date = self._get_latest_date()
        return (
            f"MACD with fast_period {fp}, slow_period {sp} and signal_period {sig} "
    ...
    def fetch_stoch(
        self, fast_k_period: int = 14, slow_k_period: int = 3, slow_d_period: int = 3
    ):
        data = self._get_data()
        ...
        last_date = self._get_latest_date()
        return (
            f"STOCH with fast_k_period {fk}, slow_k_period {sk} and slow_d_period {sd} "
    ...
    def fetch_stochrsi(
        ...
        fastk, fastd = talib.STOCHRSI(
            close, timeperiod=rl, fastk_period=kp, fastd_period=dp, fastd_matype=0
        )
        k_val, d_val = fastk[-1], fastd[-1]
        last_date = self._get_latest_date()
        return (
            f"STOCHRSI with rsi_length {rl}, stoch_length {sl}, "
    ...
    def fetch_ichimoku(
        ...
        if len(senkou_a) < kp + 1 or len(senkou_b) < kp + 1:
            print(
                f"Not enough data for Ichimoku calculation with current periods for {self.symbol}"
            )
            sys.exit(1)
        ...
        last_date = self._get_latest_date()
        return (
            f"ICHIMOKU with conversion_line_period {tp}, base_line_period {kp}, "
    ...
    def fetch_historical_adx(self, time_period: int = 14, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        adx = talib.ADX(high, low, close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": adx})
    ...
    def fetch_historical_bbands(self, time_period: int = 20, days: int = 30):
        ...
        df = pd.DataFrame(
            {"date": data.index, "value": middle, "upper": upper, "lower": lower}
        )
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
    ...
    def fetch_historical_ema(self, time_period: int = 9, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        ema = talib.EMA(close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": ema})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
        df = df.sort_index(ascending=False)
        return df, tp
    def fetch_historical_macd(
        self, macd_fast_period: int = 12, macd_slow_period: int = 26, days: int = 30
    ):
        ...
        df = pd.DataFrame(
            {"date": data.index, "value": macd, "signal": macdsignal, "hist": macdhist}
        )
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
    ...
    def fetch_historical_percent_b(self, time_period: int = 20, days: int = 30):
        ...
        df = pd.DataFrame({"date": data.index, "value": percent_b_values})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_rsi(self, time_period: int = 14, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        rsi = talib.RSI(close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": rsi})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_sma(self, time_period: int = 20, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        sma = talib.SMA(close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": sma})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_stoch(
        ...
        df = pd.DataFrame({"date": data.index, "value": slowk, "slowd": slowd})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_cci(self, time_period: int = 20, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        cci = talib.CCI(high, low, close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": cci})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_sar(self, days: int = 30):
        ...
        low = data["Low"].astype(float).to_numpy().flatten()
        sar = talib.SAR(high, low, acceleration=0.02, maximum=0.2)
        df = pd.DataFrame({"date": data.index, "value": sar})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_stochrsi(
        ...
        df = pd.DataFrame({"date": data.index, "value": fastk, "fastd": fastd})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_ichimoku(
        ...
        senkou_a = ((conv_line + base_line) / 2).shift(kp)
        senkou_b = ((high.rolling(sb).max() + low.rolling(sb).min()) / 2).shift(kp)
        df = pd.DataFrame({"date": data.index, "value": senkou_a, "senkou_b": senkou_b})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_mfi(self, time_period: int = 14, days: int = 30):
        ...
        volume = data["Volume"].astype(float).to_numpy().flatten()
        mfi = talib.MFI(high, low, close, volume, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": mfi})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_obv(self, days: int = 30):
        ...
        volume = data["Volume"].astype(float).to_numpy().flatten()
        obv = talib.OBV(close, volume)
        df = pd.DataFrame({"date": data.index, "value": obv})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_mom(self, time_period: int = 10, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        mom = talib.MOM(close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": mom})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def fetch_historical_willr(self, time_period: int = 14, days: int = 30):
        ...
        close = data["Close"].astype(float).to_numpy().flatten()
        willr = talib.WILLR(high, low, close, timeperiod=tp)
        df = pd.DataFrame({"date": data.index, "value": willr})
        df.set_index("date", inplace=True)
        df = df.dropna()
        df = df.tail(days)
        df["pct_change"] = df["value"].pct_change() * 100
    ...
    def _format_historical_data(self, df, indicator_name, params_str, days):
        ...
        result += (
            f"{indicator_name} {params_str} values for the last {actual_days} days:\n"
        )
        ...
        for date, row in df_sorted.iterrows():
            ...
            if date_str == latest_date:
                result += (
                    f"* {date_str}: {value} (LATEST {indicator_name.upper()} VALUE)\n"
                )
            else:
    ...
    def _format_price_historical_data(self, df, symbol, days):
        ...
        for date, row in df_sorted.iterrows():
            ...
            value = (
                ...
                else round(float(row["Close"]), 4)
            )
            if date_str == latest_date:
                daily_change = (
                    row["pct_change"] if not pd.isna(row["pct_change"]) else "N/A"
                )
                daily_change_str = (
                    f"{daily_change:.2f}%"
    ...
    def fetch_quote(self):
        ...
        return (
            f"Quote Summary for {data['symbol']} - {data['name']}:"
...
def get_ti_context(
    symbol: str,
    indicator_params: dict = settings.TECHNICAL_INDICATOR_DEFAULTS,
    interval: str = "1day",
    days: int = 30,
) -> str:
    ...
    adx_data, adx_tp = ti.fetch_historical_adx(
        time_period=indicator_params.get("adx_time_period", 14), days=days
    )
    ...
    bbands_data, bbands_tp = ti.fetch_historical_bbands(
        time_period=indicator_params.get("bbands_time_period", 20), days=days
    )
    bbands_history = ti._format_historical_data(
        bbands_data,
        "BBANDS (Middle Band)",
        f"with time_period of {bbands_tp} days",
        days,
    )
    formatted_output += bbands_history + "\n\n"
    ema_data, ema_tp = ti.fetch_historical_ema(
        time_period=indicator_params.get("ema_time_period", 9), days=days
    )
    ...
    macd_history = ti._format_historical_data(
        macd_data,
        "MACD",
        f"with fast_period {macd_fp}, slow_period {macd_sp} and signal_period {macd_sig}",
        days,
    )
    formatted_output += macd_history + "\n\n"
    rsi_data, rsi_tp = ti.fetch_historical_rsi(
        time_period=indicator_params.get("rsi_time_period", 14), days=days
    )
    ...
    sma_data, sma_tp = ti.fetch_historical_sma(
        time_period=indicator_params.get("sma_time_period", 20), days=days
    )
    ...
    cci_data, cci_tp = ti.fetch_historical_cci(
        time_period=indicator_params.get("cci_time_period", 20), days=days
    )
    ...
    mfi_data, mfi_tp = ti.fetch_historical_mfi(
        time_period=indicator_params.get("mfi_time_period", 14), days=days
    )

...
def obtain_market_schedule(
    start_date: datetime, end_date: datetime, market: Optional[str] = "NYSE"
) -> pd.DataFrame:
...
def get_timegpt_forecast(
    ...
    time_series_defaults: Dict = settings.TIME_SERIES_DEFAULTS,
) -> pd.DataFrame:
    """
    ...
    full_business_days = pd.bdate_range(
        start=all_dates.min(),
        end=all_dates.max() + timedelta(days=30),
        freq="B",
    )
    ...
    return df_forecast
def format_timegpt_forecast(
    forecast_df: pd.DataFrame, symbol: str, company_name: str
) -> str:
...
class TwelveDataHandler:
    def __init__(
        ...
        if (
            start_date is None
            or end_date is None
            or max_missing_data is None
            or data_folder is None
        ):
            raise ValueError(
                "start_date, end_date, max_missing_data, and data_folder must be provided."
            )
        self.start_date = start_date
        self.end_date = end_date
        self.max_missing_data = max_missing_data
        self.data_folder = data_folder
        self.market_dates = obtain_business_dates(
            start_date=self.start_date, end_date=self.end_date
        )
        self.symbol_data = {}
    def fetch_data_with_std_check(self, ticker: str) -> pd.DataFrame:
        nb_years = settings.TIME_SERIES_DEFAULTS["nb_years"]
        period_map = {1: "12mo", 2: "24mo", 3: "36mo", 4: "48mo", 5: "60mo"}
        period = period_map.get(nb_years, f"{nb_years * 12}mo")
        data = twelve_data_manager.get_time_series_data(
            ticker, interval="1day", period=period
        )
        if data.empty:
        ...
        if first_market_date not in fetched_dates.values:
            raise ValueError(
                f"Ticker {ticker} is missing the first market trading day: {first_market_date.date()}"
            )
        ...
        if len(data) >= 120:
            ...
            if std_dev == 0:
                raise ValueError(
                    f"Standard deviation is 0 for {ticker} over the last 120 trading days."
                )
        else:
            raise ValueError(
                f"Not enough data to compute standard deviation for {ticker}."
            )
    ...
    def handle_missing_data(
        self, data: pd.DataFrame, data_series: str
        ...
        if missing_dates:
            ...
            for missing_date in missing_dates:
                modified_data = self.insert_missing_date(
                    modified_data, pd.Timestamp(missing_date), "ds"
                )
    ...
    def insert_missing_date(
        data: pd.DataFrame, date: pd.Timestamp, date_column: str
    ) -> pd.DataFrame:
    ...
    def combine_data(self):
        ...
        for symbol, df in self.symbol_data.items():
            if list(df.columns) != expected_columns:
                raise ValueError(
                    f"DataFrame for {symbol} does not match the expected columns: {expected_columns}"
                )

        ...
        for row in rows:
            try:
                ...
                if "Today" in date_str:
                        ...
                        if time_part:
                            if ":" in time_part:
                                if "AM" in time_part or "PM" in time_part:
                                    ...
                                    if (
                                        "PM" in minute_sec or " PM" in minute_sec
                                    ) and hour < 12:
                                        hour += 12
                                    if (
                                        "AM" in minute_sec or " AM" in minute_sec
                                    ) and hour == 12:
                                        hour = 0
                                    article_date = datetime.datetime(
                                        year_num, month_num, day_num, hour, minute
                                    )
                                else:
                                    hour, minute = map(int, time_part.split(":"))
                                    article_date = datetime.datetime(
                                ...
                                article_date = datetime.datetime(
                                    year_num, month_num, day_num, 12, 0
                                )
                        else:
                            article_date = datetime.datetime(
                                year_num, month_num, day_num, 12, 0
                    ...
                    ":" in date_str
                    and ("AM" in date_str or "PM" in date_str)
                    and current_date_reference
                ):
                    try:
                        try:
                            time_obj = datetime.datetime.strptime(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for link in soup.find_all("a"):
                try:
                    if not link.has_attr("href") or any(
                        nav in link["href"]
                        for nav in [
                            "/search",
                            "/topic/",
                    ...
                    if "/news/" in url or "/analysis/" in url:
                        ...
                        if parent:
                            ...
                            for p_sibling in parent_siblings:
                                if hasattr(p_sibling, "find_all"):
                                    ...
                                    hasattr(p_sibling, "text")
                                    and "ago" in p_sibling.text.lower()
                                ):
                        ...
                        for time_text in time_texts:
                            try:
                                ...
                                if (
                                    ...
                                    article_date = now - datetime.timedelta(
                                        hours=time_value
                                    )
                                    found_time = True
                                    break
                        ...
                        if not found_time:
                            if (
                                now.date() - start_time.astimezone(est).date()
                            ).days == 0:
                                continue
                            article_date = now
            ...
            for item in news_items:
                if "More" in item.headline or any(
                    nav in item.url for nav in ["/topic/", "/stocks/", "/search?"]
                ):
                    continue
            ...
            if response.status_code == 200:
                ...
                for heading_text in [
                    ...
                    heading_elements = soup.find_all(
                        string=lambda s: s and heading_text in s.lower()
                    )

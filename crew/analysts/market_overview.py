...
class HistoricalMarketFetcher:
    ...
    def fetch_twelve_data_asset(self, symbol: str, days: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            df_result["pct_change"] = (
                df_result["value"].pct_change(-1) * 100
            )  # Negative because data is sorted in descending order

            return df_result.head(days)

        except Exception as e:
            raise ValueError(
                f"Error fetching data for {symbol} from TwelveData API: {str(e)}"
            )
    ...
    def fetch_vix_historical_data(self, days: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            # Calculate daily percentage change
            df["pct_change"] = (
                df["value"].pct_change(-1) * 100
            )  # Negative because data is sorted in descending order

            return df
        except Exception as e:
            raise ValueError(
                f"Error fetching VIX data from Yahoo Finance API: {str(e)}"
            )
    ...
    def _format_vix_data_simple(self, vix_data: pd.DataFrame, days: int) -> str:
        ...
        result += (
            f"VIX (CBOE Volatility Index) values for the last {actual_days} days:\n"
        )
    ...
    def _format_asset_data(
        self, asset_data: pd.DataFrame, asset_name: str, days: int
    ) -> str:
        """
        ...
        latest_date = (
            asset_data.index[0].strftime("%Y-%m-%d") if len(asset_data) > 0 else ""
        )

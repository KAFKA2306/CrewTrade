...
base.py (BaseUseCase抽象クラス)
 ↓
具体的ユースケース実装 (例: imura/imura_fund_case.py)
  ├── config.py (パラメータ)
...
### imura
井村氏運用ファンドのパフォーマンス分析と保有銘柄の追跡。

### oracle
Oracle社の収益モデルと株価への影響をシミュレーション。
...
uv run -m ai_trading_crew.use_case_runner oracle --config config/use_cases/oracle.yaml
...
1. `ai_trading_crew/{new_use_case}/` ディレクトリ作成
2. `BaseUseCase` 継承クラス実装:
   ```python
   from ai_trading_crew.base import BaseUseCase
...
3. `registry.py` に登録:
   ```python
   "new_use_case": "ai_trading_crew.new_use_case.use_case:NewUseCase"
   ```

...
from crew.loan.insights import build_insight_markdown
...
class SecuritiesCollateralLoanReporter:
    ...
    def persist(self, analysis_payload: Dict[str, object]) -> Dict[str, Path]:
        ...
        stored_paths["warning_events"] = self._save_parquet(
            "warning_events", warning_events
        )
        stored_paths["liquidation_events"] = self._save_parquet(
            "liquidation_events", liquidation_events
        )
        stored_paths["asset_breakdown"] = self._save_parquet(
        ...
        if mode == "optimization":
            ...
            for name, result in profile_results.items():
                portfolio = (
                    result.get("portfolio") if isinstance(result, dict) else None
                )
    ...
    def _build_report(
        ...
        lines.append(
            f"- Current collateral market value: ¥{summary['current_collateral_value']:.0f}"
        )
        lines.append(f"- Current loan ratio: {summary['current_loan_ratio']:.3f}")
        lines.append(
            f"- Max allowable borrowing ratio (Rakuten Securities): {self.config.ltv_limit:.2f}"
        )
        lines.append(f"- Margin call (補充) threshold: {self.config.warning_ratio:.2f}")
        lines.append(
            f"- Forced liquidation threshold: {self.config.liquidation_ratio:.2f}"
        )
        if summary["buffer_to_warning_pct"] is not None:
            lines.append(
                f"- Buffer to margin call: {summary['buffer_to_warning_pct'] * 100:.2f}% drop from current value"
            )
        if summary["buffer_to_liquidation_pct"] is not None:
            lines.append(
                f"- Buffer to forced liquidation: {summary['buffer_to_liquidation_pct'] * 100:.2f}% drop from current value"
            )
        lines.append(
        ...
        if mode == "optimization" and analysis_payload:
            ...
            if isinstance(candidate_universe, pd.DataFrame):
                corr_threshold = (
                    self.config.optimization.correlation_threshold
            ...
            constraints = (
                self.config.optimization.constraints if self.config.optimization else {}
            )
            if primary_profile:
            ...
            if variant_portfolios:
                ...
                for strategy_key, label in strategy_labels:
                    ...
                    portfolio_df = (
                        ...
                        if isinstance(variant.get("portfolio"), pd.DataFrame)
                        else None
                    )
                    ...
                    if (
                        ...
                        for _, row in portfolio_df.iterrows():
                            ...
                            price_display = (
                                f"¥{price:,.0f}" if pd.notna(price) else "N/A"
                            )
                            formatted_rows.append(
            ...
            if hedged_excluded or volatility_excluded or drawdown_excluded:
                ...
                if max_vol_threshold is not None:
                    threshold_notes.append(
                        f"max volatility ≤ {max_vol_threshold * 100:.1f}%"
                    )
                if max_dd_threshold is not None:
                    threshold_notes.append(
                        f"max drawdown ≥ -{max_dd_threshold * 100:.1f}%"
                    )
                if threshold_notes:
                    lines.append(
                        f"- Applied Asset Filters: {', '.join(threshold_notes)}"
                    )
                if hedged_excluded:
                    lines.append("")
            ...
            if backtest_start and backtest_end:
                ...
                lines.append(
                    f"**Period**: {backtest_start.date()} to {backtest_end.date()} ({duration_years:.1f} years)"
                )
            else:
            ...
            if isinstance(ranked_etfs, pd.DataFrame) and not ranked_etfs.empty:
                ...
                for i, (_, row) in enumerate(ranked_etfs.head(10).iterrows(), 1):
                    ...
                    expense_display = (
                        f"{expense * 100:.2f}%"
                        if expense is not None and not pd.isna(expense)
                        else "N/A"
                    )
                    lines.append(
        ...
        if mode == "optimization" and "category" in asset_breakdown.columns:
            ...
            category_summary["weight"] = (
                category_summary["market_value"]
                / category_summary["market_value"].sum()
            )
        ...
        for _, row in asset_breakdown_sorted.head(display_count).iterrows():
            ...
            ret_display = (
                f"{ret * 100:.2f}%" if ret is not None and not pd.isna(ret) else "N/A"
            )
            vol = row.get("annual_volatility")
            vol_display = (
                f"{vol * 100:.2f}%" if vol is not None and not pd.isna(vol) else "N/A"
            )
            sharpe = row.get("sharpe_ratio")
            sharpe_display = (
                f"{sharpe:.3f}" if sharpe is not None and not pd.isna(sharpe) else "N/A"
            )
            lines.append(
        ...
        if annual_asset_returns:
            ...
            for row in annual_asset_returns:
                ...
                vol_display = (
                    f"{row['annual_volatility'] * 100:.2f}%"
                    if row["annual_volatility"] is not None
                    else "N/A"
                )
                sharpe_value = row.get("sharpe_ratio")
                sharpe_display = (
                    f"{sharpe_value:.3f}" if sharpe_value is not None else "N/A"
                )
                lines.append(
        ...
        if isinstance(forward_test, dict):
            ...
            if start and end:
                ...
                duration_years = duration_days / 365.25
                lines.append(
                    f"**Period**: {start.date()} to {end.date()} ({duration_years:.1f} years)"
                )
            lines.append(
            ...
            if summary_forward.get("cumulative_return") is not None:
                lines.append(
                    f"| Cumulative Return | {summary_forward['cumulative_return'] * 100:.2f}% |"
                )
            if summary_forward.get("annualized_return") is not None:
                lines.append(
                    f"| Annualized Return | {summary_forward['annualized_return'] * 100:.2f}% |"
                )
            if summary_forward.get("annualized_volatility") is not None:
                ...
                lines.append(
                    f"| Annualized Volatility | {fw_vol * 100:.2f}%{vol_note} |"
                )
            if summary_forward.get("max_drawdown") is not None:
                lines.append(
                    f"| Max Drawdown | {summary_forward['max_drawdown'] * 100:.2f}% |"
                )

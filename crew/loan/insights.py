...
def build_insight_markdown(analysis_payload: Dict[str, object]) -> str:
    ...
    if mode == "optimization":
        ...
        if optimized_portfolio is not None:
            if primary_profile:
                lines.append(
                    f"- Selected ETFs ({primary_profile}): {len(optimized_portfolio)}"
                )
            else:
        ...
        if volatility_excluded:
            ...
            if max_asset_volatility is not None:
                ...
                lines.append(
                    f"- Excluded {len(volatility_excluded)} ETF(s) above volatility threshold"
                )
        ...
        lines.append(
            f"- Portfolio annual volatility: {opt_metrics.get('annual_volatility', 0) * 100:.2f}%"
        )
        lines.append(
            f"- Portfolio Sharpe ratio: {opt_metrics.get('sharpe_ratio', 0):.3f}"
        )
    ...
    lines.append(
        f"- Buffer to 70%: {summary['buffer_to_warning_pct'] * 100:.2f}% drop"
        if summary["buffer_to_warning_pct"] is not None
        else "- Buffer to 70%: N/A"
    )
    lines.append(
        f"- Buffer to 85%: {summary['buffer_to_liquidation_pct'] * 100:.2f}% drop"
        if summary["buffer_to_liquidation_pct"] is not None
        else "- Buffer to 85%: N/A"
    )
    ...
    if not scenario_table.empty:
        ...
        scenario_table["loan_ratio"] = scenario_table["loan_ratio"].map(
            lambda x: f"{x:.3f}"
        )
        scenario_table["breach_warning"] = scenario_table["breach_warning"].map(
            lambda x: "Yes" if x else "No"
        )
        scenario_table["breach_liquidation"] = scenario_table["breach_liquidation"].map(
            lambda x: "Yes" if x else "No"
        )
    lines.append(
        _format_table(

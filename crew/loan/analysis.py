...
class SecuritiesCollateralLoanAnalyzer:
    ...
    def _evaluate_optimization_mode(
        ...
        def _is_hedged_label(value: str) -> bool:
            ...
            if (
                "hedged" in text_lower
                and "unhedged" not in text_lower
                and "hedge-free" not in text_lower
            ):
                return True
            if "ヘッジ" in text:
                if any(
        ...
        for column in ("name", "description"):
            if column in etf_master.columns:
                hedged_mask = hedged_mask | etf_master[column].fillna("").apply(
                    _is_hedged_label
                )
        ...
        if hedged_tickers:
            ...
            correlation_matrix = correlation_matrix.drop(
                index=list(hedged_tickers),
                columns=list(hedged_tickers),
                errors="ignore",
            )
            hedged_assets = (
                etf_master.loc[
                    etf_master["ticker"].isin(hedged_tickers), ["ticker", "name"]
                ]
        ...
        if (
            ...
            mask_volatility = (
                risk_metrics["annual_volatility"] <= max_asset_volatility
            ).fillna(False)
            excluded_vol_df = risk_metrics.loc[
                ~mask_volatility, ["ticker", "name"]
            ].drop_duplicates(subset=["ticker"])
        ...
        if max_asset_drawdown is not None and "max_drawdown" in risk_metrics.columns:
            ...
            excluded_drawdown_df = risk_metrics.loc[
                ~mask_drawdown, ["ticker", "name"]
            ].drop_duplicates(subset=["ticker"])
        ...
        if self.config.optimization.profiles:
            ...
            profile_configs = [
                OptimizationProfile(
                    name="max_sharpe",
                    objective_weights={"sharpe": 1.0, "volatility": 0.0},
                ),
                OptimizationProfile(
                    name="balanced", objective_weights=objective_weights
                ),
                OptimizationProfile(
                    name="min_volatility",
        ...
        for profile in profile_configs:
            ...
            candidate_universe = etf_screening.select_candidate_universe(
                ...
                priority_indices=self.config.optimization.priority_indices
                if self.config.optimization
                else None,
            )
        ...
        merge_cols = [
            col
            for col in ["ticker", "category", "expense_ratio", "name", "provider"]
            if col in metadata_subset_primary.columns
        ]
        if merge_cols:
            enriched = metadata_subset_primary[merge_cols].drop_duplicates(
                subset=["ticker"]
            )
            asset_breakdown = asset_breakdown.merge(enriched, on="ticker", how="left")
            asset_breakdown["description"] = asset_breakdown["name"].fillna(
                asset_breakdown["description"]
            )
        portfolio_merge_cols = [
            col
            for col in ["ticker", "weight", "weight_realized"]
            if col in optimized_portfolio.columns
        ]
        ...
        if risk_merge_cols:
            risk_enriched = risk_metrics[
                risk_metrics["ticker"].isin(asset_breakdown["ticker"])
            ][risk_merge_cols].drop_duplicates(subset=["ticker"])
            asset_breakdown = asset_breakdown.merge(
        ...
        manual_result.update(
            {
                ...
                "annual_asset_returns": self._compute_annual_asset_returns(
                    selected_prices, asset_breakdown
                ),
                "annual_portfolio_returns": self._compute_annual_portfolio_returns(
                    manual_result["portfolio_value"]
                ),
    ...
    def _evaluate_manual_mode(
        ...
        warning_events = self._extract_events(
            loan_ratio_series, self.config.warning_ratio, "warning"
        )
        liquidation_events = self._extract_events(
        ...
        asset_breakdown = pd.DataFrame(
            {
                ...
                "description": [
                    asset.description or "" for asset in self.config.collateral_assets
                ],
                "latest_price": [
                    float(prices[asset.ticker].iloc[-1])
    ...
    def _run_scenarios(self, latest_value: float) -> List[Dict[str, float]]:
        ...
        for scenario in self.config.scenarios:
            ...
            ratio = (
                self.config.loan_amount / effective_value
    ...
    def _compute_annual_asset_returns(
        ...
        if "description" in asset_breakdown.columns:
            name_map = (
                asset_breakdown.set_index("ticker")["description"].fillna("").to_dict()
            )
    ...
    def _validate_portfolio_drawdown(
        ...
        return {
            ...
            "compliant": compliant,
            "breach_amount": abs(max_drawdown) - max_dd_constraint
            if not compliant
            else 0.0,
        }
    ...
    def _apply_core_satellite_strategy(
        ...
        if needs_core_rebalance:
            ...
            tier1_tickers = (
                set(priority_indices.get("tier1", [])) if priority_indices else set()
            )
            tier2_tickers = (
            ...
            satellite_total_weight = (
                satellite_portfolio["weight"].sum()
            ...
            metadata = PortfolioMetadata(
                ...
                rebalance_year=0,
                valid_until=(
                    anchor_date + pd.DateOffset(years=cs_config.core_rebalance_years)
                ).isoformat(),

...
def rank_etfs(
    ...
    active = [
        ...
        if key in metric_map and metric_map[key] in df.columns
    ]
    if active:
        ...
        if total_weight == 0:
            weights_array = np.full_like(
                weights_array, 1 / len(weights_array), dtype=float
            )
        else:
...
def select_candidate_universe(
    ...
    for ticker in tickers:
        ...
        if not priority_in_component.empty:
            tier1_match = priority_in_component[
                priority_in_component["ticker"].isin(tier1_set)
            ]
...
def _collect_component(seed: str, corr: pd.DataFrame, threshold: float) -> Set[str]:
    ...
    while to_visit:
        ...
        neighbors = [
            col for col, value in row.items() if value >= threshold and col != current
        ]

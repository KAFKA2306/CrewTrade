...
def optimize_collateral_portfolio(
    ...
    expense_lookup = (
        etf_master.set_index("ticker")["expense_ratio"].to_dict()
        if not etf_master.empty
        else {}
    )
    expense_values_all = [
        ...
        for _ in range(sample_size):
            if subset_capacity < n_assets:
                ...
                subset_indices = np.random.choice(
                    n_assets, size=subset_size, replace=False
                )
            else:
            ...
            if max_category_weight is not None and category_lookup:
                ...
                for idx, weight in zip(subset_indices, candidate_weights):
                    ...
                    category_weights[category] = (
                        category_weights.get(category, 0.0) + weight
                    )
                if any(
    ...
    if best_portfolio is None:
        ...
        if fallback_subset <= 0:
            raise ValueError(
                "Unable to construct fallback portfolio; no assets available."
            )
    ...
    if total_allocated > 0:
        portfolio_df["weight_realized"] = (
            portfolio_df["allocation_value"] / total_allocated
        )
    else:

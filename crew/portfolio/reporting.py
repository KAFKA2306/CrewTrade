    ...
    def persist(self, analysis_payload: Dict) -> Dict:
        ...
        if self.config is not None:
            ...
            report_lines.append(
                "- **最適化目的関数ウェイト:** "
                + ", ".join(
                    f"{k} {v * 100:.0f}%" for k, v in opt_conf.objective_weights.items()
                )
            )
            report_lines.append(
                "- **制約条件:** "
            ...
            report_lines.append("- **リバランス頻度（検証時）:** 四半期（Q）")
            report_lines.append("- **訓練期間 / テスト期間:** 5年 / 2年（ローリング）")
            ...
            if walk_forward_results:
                ...
                if summary:
                    report_lines.append(
                        f"- **平均アウトオブサンプル Sharpe:** {summary.get('avg_sharpe', 0):.3f} ± {summary.get('std_sharpe', 0):.3f}"
                    )
                    report_lines.append(
                        f"- **平均年率リターン:** {summary.get('avg_annual_return', 0) * 100:.2f}%"
                    )
                    report_lines.append(
                        f"- **平均最大ドローダウン:** {summary.get('avg_max_drawdown', 0) * 100:.2f}%"
                    )
                    report_lines.append(

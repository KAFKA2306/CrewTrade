...
def _optimize_weights(
    ...
    for sample_size in opt_cfg.sensitivity_sample_sizes:
        if sample_size == opt_cfg.sample_size:
            continue
        sensitivity = _run_random_search(
            returns, tickers, sample_size, opt_cfg, seed=42 + sample_size
        )
        if sensitivity is None:
            continue
        sensitivity_metrics = _compute_performance_metrics(
            returns, sensitivity["weights"], opt_cfg.risk_free_rate
        )

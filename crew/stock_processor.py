...
from crew.analysts.stock_headlines_fetcher import get_news_context
from crew.analysts.technical_indicators import get_ti_context
from crew.analysts.timegpt import format_timegpt_forecast
...
from crew.crew import (
    AiArticlesPickerCrew,
    DayTraderAdvisorCrew,
    StockComponentsSummarizeCrew,
)
...
async def process_stock_symbol(
    ...
    stock_news = await get_stock_news(
        symbol,
        os.path.join(
            AGENT_INPUTS_FOLDER, today_str_no_min, f"{symbol}_{RELEVANT_ARTICLES_FILE}"
        ),
    )
    with open(
        os.path.join(AGENT_INPUTS_FOLDER, today_str_no_min, f"{symbol}_stock_news.txt"),
        "w",
    ) as f:
        f.write(stock_news)
    ...
    with open(
        os.path.join(AGENT_INPUTS_FOLDER, today_str_no_min, f"{symbol}_stocktwits.txt"),
        "w",
    ) as f:
    ...
    technical_summary = read_summary_file(
        symbol, "technical_indicator_summary_report.md"
    )
    fundamental_summary = read_summary_file(
    ...
    market_sentiment_summary = read_summary_file(
        settings.STOCK_MARKET_OVERVIEW_SYMBOL, "sentiment_summary_report.md"
    )
    market_technical_summary = read_summary_file(
        settings.STOCK_MARKET_OVERVIEW_SYMBOL, "technical_indicator_summary_report.md"
    )
    market_timegpt_summary = read_summary_file(
        settings.STOCK_MARKET_OVERVIEW_SYMBOL, "timegpt_forecast_summary_report.md"
    )
...
def process_stock_symbol_sync(
    ...
    return asyncio.run(
        process_stock_symbol(
            symbol, vix_data, global_market_data, additional_agents, additional_tasks
        )
    )

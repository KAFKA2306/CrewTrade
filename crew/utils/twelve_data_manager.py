...
class TwelveDataManager:
            ...
            try:
                ...
                if data.get("status") == "error":
                    ...
                    if (
                        ...
                        "run out of API credits" in error_msg
    ...
    def get_time_series_data(
        self, symbol: str, interval: str = "1day", period: str = "4mo"
    ) -> pd.DataFrame:
        """
        ...
        if (
            cache_key in self._cached_data
            and current_time - self._last_fetch_times.get(cache_key, 0)
            < self._cache_ttl
        ):
    ...
    def get_quote_data(self, symbol: str) -> Dict[str, Any]:
        ...
        current_time = time.time()
        if (
            symbol in self._cached_quotes
            and current_time - self._last_quote_fetch_times.get(symbol, 0)
            < self._cache_ttl
        ):
            return self._cached_quotes[symbol]
        quote_url = (
            f"https://api.twelvedata.com/quote?symbol={symbol}&apikey={self.api_key}"
        )
        ...
        if not quote_data or quote_data.get("status") == "error":
            ...
            if previous and "close" in current and "close" in previous:
                ...
                percent_change = round(
                    (current_close - prev_close) / prev_close * 100, 2
                )
            quote_data = {
                "symbol": symbol,
    ...
    def get_company_name(self, symbol: str) -> str:
        ...
        current_time = time.time()
        if (
            symbol in self._cached_company_names
            and current_time - self._last_company_fetch_times.get(symbol, 0)
            < self._cache_ttl
        ):

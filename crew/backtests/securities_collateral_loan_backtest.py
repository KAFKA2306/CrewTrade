...
from crew.use_case_runner import build_config
@dataclass
...
class SecuritiesCollateralLoanBacktester:
    ...
    def run(self, anchors: List[pd.Timestamp] | None = None) -> List[BacktestResult]:
        ...
        for year_index, anchor in enumerate(anchors):
            ...
            forward_test = self._compute_forward_test(
                anchor, analysis_payload, pipeline
            )
    ...
    def _compute_forward_test(
        ...
        annualized_return = (
            (end_value / start_value) ** (365 / days) - 1.0 if start_value > 0 else None
        )
        daily_returns = portfolio_values.pct_change().dropna()
        annualized_vol = (
            float(daily_returns.std() * (252**0.5)) if not daily_returns.empty else None
        )
    ...
    def _load_previous_portfolio(
        self, anchor: pd.Timestamp
    ) -> Tuple[pd.DataFrame | None, PortfolioMetadata | None]:
...
def build_anchor_dates(
    years: int, reference_date: datetime | None = None
) -> List[pd.Timestamp]:
...
def main() -> None:
    ...
    parser.add_argument(
        "name", help="Use case name", default="securities_collateral_loan"
    )

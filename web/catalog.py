...
CASE_CATALOG: Final[dict[str, dict[str, str]]] = {
    "credit": {
        ...
        "period": "公開利用許諾を確認できるまで数値停止",
        "status": "ライセンス待ち",
        "summary": "ETF代理系列を廃止し、ICE BofA OASの公開再配布権を記録できるまで数値とシグナルを停止します。",
        "question": "信用リスクへの追加補償を、利用権を満たす一次データで評価できるか",
        "warning": "FREDで閲覧可能であることを第三者サイトでの再配布許可と解釈しません。",
        "change_summary": "旧OAS数値とETF代理評価を撤回し、ライセンス待ちの統制状態へ変更。",
        "accent": "apricot",
    },
    "imura": {
        "title": "ファンド・指数比較",
    ...
    "yield_spread": {
        ...
        "period": "現在のカーブと過去推移",
        "status": "正準稼働",
        "summary": "米国財務省の名目・実質カーブを別系列として扱い、期間構造と期待インフレを分離します。",
        "question": "金利差の変化は、どの期待とリスクプレミアムを反映しているか",
        "warning": "名目金利差だけで為替方向を断定せず、実質金利、ヘッジ費用、ポジションを確認します。",
        "change_summary": "FRED依存を外し、米国財務省の名目・実質パー・イールドへ統合。",
        "accent": "mint",
    },
}

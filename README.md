...
CrewTradeは、一次データの取得、定量計算、評価、文章による解釈を分離し、再検証可能な分析スナップショットとして公開する金融調査ワークスペースです。
...
金融データを多く集めることではなく、**利用者が「取得できた」「比較できる」「公開してよい」「まだ使ってはいけない」を見分け、根拠のある分析だけへ進める調査体験**を作ります。

CrewTradeの目的は、全テーマに常に数字を表示することではありません。利用可能性・比較可能性・利用権・秘密性の条件が揃わない数字は出さず、何が不足しているかを状態として残します。

## Design philosophy

- 欠損値を古い固定値やproxyで埋めない
- data availabilityとdata permissionを別々に確認する
- 異なる定義・単位・観測時点を同じ指標として並べない
- 接続不能を失敗として隠さず、理由付きのcontrolled stateとして見せる
- Pagesの`OK`を「全テーマに数値がある」と誤解させない
- 一次事実、独自導出値、モデル出力、文章解釈を分離する

## Why / 差別化

一般的なmarket dashboardが「数字を表示すること」を中心に置くのに対し、CrewTradeは**その数字を今使ってよい理由、または使ってはいけない理由を確認できること**を中心に置きます。
...
- `canonical_active`: 公開分析へ使える正準入力が揃っている
- `governed_blocked`: 接続・契約・利用権などの条件不足を理由付きで停止している
- `governed_partial`: 一部の公開入力だけ利用可能で、残りの不足を明示している
- `private_input_required`: 非公開契約・口座状態などが必要で、公開Pagesへ値を出さない

取得経路、利用権、比較可能性、秘密性のいずれかが不足する場合は数値を推測しません。Pagesの`OK`は「全テーマが数値を出している」ことではなく、到達可能な公開分析が正準稼働し、残りが明示的に統制されていることを意味します。

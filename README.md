...
取得経路、利用権、比較可能性、秘密性のいずれかが不足する場合は数値を推測せず、理由付きで停止します。Pagesの`OK`は「全テーマが数値を出している」ことではなく、公開可能な分析が正準稼働し、残りが統制された状態にあることを意味します。
...
1. 米国財務省・SECの正準一次データを取得
2. raw・SHA-256・Parquet・DuckDBへ保存
3. 共通品質検査を実行
4. 公開用の正準接続済み3テーマを生成
5. 7テーマの停止・一部利用・非公開入力状態を更新
6. GitHub Pagesを再生成・監査・公開

SECのUser-Agentは公開リポジトリの連絡先をworkflowで宣言します。公開用の稼働に秘密情報は不要です。
...
uv run crew-data query "select * from gold_sec_company_facts_latest"
